...
$trips = $pdo->prepare("
    SELECT t.TripId,t.TripDate,t.InvoiceNo,t.FromLocation,t.ToLocation,
           t.FreightAmount, t.CashAdvance, t.OnlineAdvance, t.AdvanceAmount, t.TDS,t.LabourCharge, t.HoldingCharge, t.OtherCharge,
           (t.FreightAmount + t.LabourCharge + t.HoldingCharge + t.OtherCharge) AS TotalAmount,
           (t.FreightAmount - t.AdvanceAmount - t.TDS + t.LabourCharge + t.HoldingCharge + t.OtherCharge) AS NetAmount,
           v.VehicleNumber,
...
$totalTds = 0; // calculated from trips below
$netTotal = floatval($bill['NetBillAmount']); // already correct (Freight - Adv - TDS)
...
<body>
  ...
  <div class="page">
    ...
    <div class="info-grid">
      <div class="info-left">
        <div class="field-row"><label>Name :</label>
          <div class="field-val"><?= s($bill['PartyName']) ?></div>
        </div>
        <div class="field-row"><label>Address :</label>
          <div class="field-val"><?= s($bill['City'] ?? '') ?><?= $bill['State'] ? ' - ' . s($bill['State']) : '' ?></div>
        </div>
      </div>
      <div class="info-right">
        <div class="field-row"><label>Bill No.</label>
          <div class="field-val"><span class="bill-no"><?= s($bill['BillNo']) ?></span></div>
        </div>
        <div class="field-row"><label>Date :</label>
          <div class="field-val"><?= date('d / m / Y', strtotime($bill['BillDate'])) ?></div>
        </div>
      </div>
    </div>
    ...
    <table class="trips-table">
      <thead>
        <tr>
          <th width="24">Sr.<br>No.</th>
          <th width="48">Date</th>
          <th width="58">Truck No.</th>
          <th width="40">L.R.</th>
          <th width="48">From</th>
          <th width="48">To</th>
          <th width="36">Wt.<br>(MT)</th>
          <th width="38">Rate</th>
          <th width="48">Freight</th>
          <th width="38">Other<br>Charges</th>
          <th width="48">Total</th>
          <th width="38">Cash<br>Adv.</th>
          <th width="38">Online<br>Adv.</th>
          <th width="32">TDS</th>
          <th width="52">Net Amt.</th>
        </tr>
      </thead>
      <tbody>
        <?php
        ...
        for ($i = 0; $i < $totalRows; $i++):
          ...
          if ($tr) {
            ...
            $fr   = floatval($tr['FreightAmount']);
            $lab  = floatval($tr['LabourCharge']);
            $hold = floatval($tr['HoldingCharge']);
            $oth  = floatval($tr['OtherCharge']);

            $cash = floatval($tr['CashAdvance']);
            $onl  = floatval($tr['OnlineAdvance']);
            $adv  = floatval($tr['AdvanceAmount']);
            $tds  = floatval($tr['TDS']);

            $wt   = floatval($tr['TotalWeight']);
            $rate = floatval($tr['AverageRate']);
            $otherCharges = $lab + $hold + $oth;
            $tot  = $fr + $lab + $hold + $oth;
            $net  = $tot - $adv - $tds;
        ...
        ?>
          <tr style="height:18px">
            ...
            <td class="td-right"><?= $tr && $wt > 0 ? number_format($wt, 3) : '' ?></td>
        ...
        <tr class="total-row">
          ...
          <td class="td-right"><?= rs($gother) ?></td>
          <td class="td-right"><?= rs($gFr + $gother) ?></td>
          <td class="td-right"><?= rs($gCash) ?></td>
          <td class="td-right"><?= rs($gOnline) ?></td>
          <td class="td-right"><?= rs($gTds) ?></td>
      ...
      <div class="totals-area">
        <table>
          ...
          <tr style="font-weight:800;">
            <td>TOTAL</td>
            <td>Rs. <?= rs($gFr + $gLab + $gHold + $gOth) ?></td>
          </tr>
          <?php if ($gCash > 0): ?><tr>
              <td>(-) Cash Adv.</td>
              <td>Rs. <?= rs($gCash) ?></td>
          ...
          <tr class="sig-row">
            <td colspan="2">For, Shama Roadlines</td>
          </tr>
        </table>
      </div>
    </div>
    ...
    <div class="words-row">
      <b>Rs. In Word :</b>
      <span><?= s(numToWords(intval($gNet)) . ' Only') ?></span>
    </div>
