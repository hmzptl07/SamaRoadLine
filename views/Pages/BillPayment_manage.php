        ...
        <div class="border rounded p-2 mb-3 bg-light">
          <div class="row g-0 text-center mb-2">
            <div class="col-4 border-end">
              <div class="fs-11 text-muted">Bill No.</div>
              <div class="fw-bold fs-13" id="pm_billno">—</div>
            </div>
            <div class="col-4 border-end">
              <div class="fs-11 text-muted">Party</div>
              <div class="fw-bold fs-12" id="pm_party">—</div>
            </div>
            <div class="col-4">
              <div class="fs-11 text-muted">Remaining</div>
              <div class="fw-bold text-danger fs-15" id="pm_rem">Rs. 0</div>
            </div>
          </div>
...
<script>
  ...
  function submitPay() {
    ...
    fetch('BillPayment_manage.php', {
      ...
      if (res.status === 'success') {
        ...
        if (res.commAuto) msg += ' Commission auto-marked Received.';
        Swal.fire({
          icon: 'success',
          title: msg,
          toast: true,
  ...
  function toggleOwnerAll(cb) {
    $('.owner-chk:visible').prop('checked', cb.checked);
    updateOwnerSel();
  }
  ...
  function confirmOwnerMark() {
    ...
    fetch('BillPayment_manage.php', {
      ...
      if (res.status === 'success') {
        ...
        Swal.fire({
          icon: 'success',
          title: res.count + ' Commissions marked Received!',
          toast: true,
          position: 'top-end',
          showConfirmButton: false,
          timer: 2500
