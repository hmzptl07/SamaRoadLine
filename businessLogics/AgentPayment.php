...
class AgentPayment {
     ...
     * Balance = NetAmount - SUM(payments received from agent)
     */
    public static function getAllTripsWithPaymentStatus(?int $agentId = null): array {
        global $pdo;

        $where  = $agentId ? "AND t.AgentId = :agentId" : "";
        ...
        $stmt = $pdo->prepare("
            SELECT
                ...
                COUNT(ap.PaymentId)                  AS PaymentCount
            FROM TripMaster t
        ...
        foreach ($rows as &$r) {
            ...
            if ($net > 0 && $paid >= $net)  $r['PaymentStatus'] = 'Paid';
            elseif ($paid > 0)              $r['PaymentStatus'] = 'PartiallyPaid';
    ...
    public static function getByTrip(int $tripId): array {
        global $pdo;
        $stmt = $pdo->prepare(
            "SELECT * FROM AgentPayment WHERE TripId = ? ORDER BY PaymentDate ASC, PaymentId ASC"
        );
        $stmt->execute([$tripId]);
    ...
    public static function deletePayment(int $paymentId): array {
        global $pdo;
        try {
            $pdo->prepare("DELETE FROM AgentPayment WHERE PaymentId = ?")->execute([$paymentId]);
            return ['status' => 'success'];
        } catch (Exception $e) {
            return ['status' => 'error', 'msg' => $e->getMessage()];
        }
    ...
    /** Agent-wise summary (for summary table) */
