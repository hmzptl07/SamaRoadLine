...
class Commission {
    ...
    public static function save(PDO $pdo, int $tripId, float $amount, string $recFrom): array {
        // Auto-detect RecoveryFrom:
        // PaidDirectly trip → Owner (manual recovery needed)
        // Normal trip       → Party (auto-received when bill paid / owner fully paid)
        $tr = $pdo->prepare("SELECT FreightPaymentToOwnerStatus FROM TripMaster WHERE TripId = ?");
        $tr->execute([$tripId]);
        $trip = $tr->fetch(PDO::FETCH_ASSOC);
        $recFrom = ($trip && $trip['FreightPaymentToOwnerStatus'] === 'PaidDirectly')
            ? 'Owner'
            : 'Party';
        try {
